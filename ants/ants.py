...
class Ant(Insect):
    ...
    def double(self):
        ...
        while exit_place:
            current_ant = exit_place.ant
            if current_ant:
                if current_ant.checker == False:
                    current_ant.damage *= 2
                    current_ant.checker = True
                if current_ant.is_container:
                    if current_ant.ant_contained:
                        if current_ant.ant_contained.checker == False:
                            current_ant.ant_contained.damage *= 2
                            current_ant.ant_contained.checker = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        new_entrance = self.place
        counter = 0
        while new_entrance.is_hive != True:
            if len(new_entrance.bees) !=  0 and self.max_range >= counter >= self.min_range:
                return random_bee(new_entrance.bees)
            counter += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        num_bees = list(self.place.bees)
        super().reduce_health(amount)
        if self.health <= 0:
            for bee in num_bees:
                bee.reduce_health(amount + self.damage)
        else:
            [bee.reduce_health(amount) for bee in num_bees]       
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_timer != 0:
            ...
            chosen_bee = random_bee(bee_list)
            if chosen_bee:
                Insect.reduce_health(chosen_bee, chosen_bee.health)
...
# END Problem 12
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0:
            ants_lose()
        super().reduce_health(amount)
        exit_checker = self.place.exit
        while exit_checker:
            exit_checker = exit_checker.exit
        if exit_checker == None and len(self.place.bees) > 0:
            ants_lose()
    
    def remove_from(self, place):
        if isinstance(place.ant, QueenAnt) or isinstance(place.ant.ant_contained, QueenAnt):
            return
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.was_scared == 2 and self.scare_state != 2: 
            if self.place.is_hive:
                return
            else:     
                if gamestate.time % 2 != 0 and self.is_slow > 0:
                    destination = self.place.exit
                else:
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC      
        if self.was_scared == 0:
            self.was_scared += length
